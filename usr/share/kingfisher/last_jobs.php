...
$page->content = '<div id="titlemain">'.$tmplang->lang['last_jobs'].'</div><br /><br />';
...
$conn->sqlquery = "SELECT job_date, job_time, username, users.id as users_id, title, printername, printers.id as printers_id, paperformatname, paperformats.id as paperformats_id, job_size, copies, number_of_pages, total_pages from printlogs, users, printers, paperformats where printlogs.users_id=users.id and printlogs.printers_id=printers.id and printlogs.paperformats_id=paperformats.id order by job_date desc, job_time desc limit 20";
